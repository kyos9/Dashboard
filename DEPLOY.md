...
헤더의 **🔔** → **이 기기에서 알림 켜기** → 브라우저가 물으면 **허용**. **시험 알림 보내기**로
바로 확인할 수 있습니다. 서버에서 따로 설정할 것은 없습니다 — 알림에 쓰는 서버 키는 앱이 처음
뜰 때 `/data/vapid.pem` 에 알아서 만듭니다 (로그인 서명 키 옆, 같은 볼륨).

- **켜는 건 기기마다**입니다. 폰에서 켜고 PC에서 안 켜면 폰에만 옵니다. 받을 종류(매수
  시그널·비중조절·리뷰, 관리자는 가입 신청)는 같은 창에서 고르고, 어느 기기에서 골라도 같습니다.
- **아이폰·아이패드는 홈 화면에 추가한 앱에서만** 됩니다 (iOS 16.4 이상). 사파리 탭에서 🔔 를
  열면 그 안내가 나옵니다.
- **언제 오나** — 장 마감 뒤 시세를 받은 다음입니다. 미국 종목은 한국 아침(07:30쯤), 한국
  종목은 오후(16:30쯤). 같은 신호로 매일 울리지 않고, **새로 생긴 날 한 번**만 옵니다.
- 가입 신청은 누가 신청하는 즉시 관리자에게 갑니다.
- **나가기를 누르면 그 기기의 알림도 꺼집니다** — 남의 폰·공용 PC에서 내 알림이 계속 오지 않게.

> `/data/vapid.pem` 이 사라지면(볼륨을 지웠을 때) 새 키가 만들어지고, 켜 두었던 기기는 각자
> 앱을 한 번 열 때 새 키로 다시 구독합니다. 앱을 열기 전까지는 알림이 안 옵니다.
...
| 🔔 를 눌렀는데 "홈 화면에 추가한 앱에서만" | 아이폰 사파리 탭입니다. 공유 → 홈 화면에 추가 → 그 아이콘으로 열고 다시 켜세요 (8번) |
| 🔔 에 "알림이 차단되어 있습니다" | 예전에 "차단"을 눌렀습니다. 브라우저의 사이트 설정 → 알림 → 허용으로 바꾸고 창을 다시 여세요 |
| "브라우저가 알림 등록을 거절했습니다" | 시크릿 창이거나 브라우저(브레이브 등)가 푸시를 꺼 두었습니다. 일반 창의 크롬·사파리·엣지에서 켜세요 |
| 알림을 켰는데 시그널 알림이 안 옴 | 먼저 **시험 알림**이 오는지 보세요. 오면 정상입니다 — 매일 알림은 **새로** 뜬 신호만, 장 마감 뒤 갱신 다음에 옵니다 (8번). 시험 알림도 안 오면 폰의 알림 설정에서 브라우저(또는 설치한 앱)의 알림이 꺼져 있지 않은지 |
