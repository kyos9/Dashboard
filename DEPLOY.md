...
> 데 그만큼 걸립니다). 화면 오른쪽 위의 커밋 해시가 안 바뀌었으면 Actions 탭에서
...
| 올렸는데 화면이 그대로 | `git pull` 만 하고 `docker compose pull` 을 안 했거나, 이미지가 아직 만들어지는 중입니다 (9번). 화면 오른쪽 위에 `0.11.0 (abc1234)` 처럼 **커밋 해시**가 뜨므로, 그게 올린 커밋과 같은지 보면 됩니다 |
