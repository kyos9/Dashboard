...
- **추가**: 콘솔의 **테스트 사용자**에 넣고, `.env` 의 `ALLOWED_GOOGLE_EMAILS` 에도 넣습니다
  (쉼표로 구분). 그리고 `docker compose --profile https up -d`. 둘 다 있어야 들어옵니다 —
  콘솔은 구글 화면을, `.env` 는 우리 앱을 지킵니다.
- **빼기**: `.env` 에서 지우고 `up -d`. 다음 로그인부터 막힙니다 (이미 들어와 있는 기기는
  쪽지가 만료될 때까지(최대 30일) 유지됩니다). 데이터는 남습니다 — 지우려면 본인이
  화면에서 `탈퇴` 를 누릅니다.
...
| 로그인 후 "이 구글 계정은 아직 들어올 수 없습니다" | `.env` 의 `ALLOWED_GOOGLE_EMAILS` 에 그 Gmail이 없습니다. 넣고 `up -d` (12번) |
