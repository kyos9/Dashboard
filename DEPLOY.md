...
| 올렸는데 화면이 그대로 | `git pull` 만 하고 `docker compose pull` 을 안 했거나, 이미지가 아직 만들어지는 중입니다 (9번). 화면 오른쪽 위에 `v0.11.0 · 9/22 01:11` 처럼 **이미지를 만든 시각**이 뜹니다. 방금 올린 것보다 이르면 아직 안 받은 겁니다 (배지에 마우스를 올리면 커밋 해시도 나옵니다) |
